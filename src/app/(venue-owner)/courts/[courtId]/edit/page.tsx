...
export default function EditCourtPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([sportsData, courtData]) => {
        setSports(sportsData.sports ?? sportsData ?? []);
        const court = courtData.court ?? courtData;
        if (court) {
          setName(court.name ?? '');
          setSportTypeId(typeof court.sportType === 'object' ? court.sportType._id : court.sportType ?? '');
          setDescription(court.description ?? '');
          setCapacity(court.capacity ?? 4);
          setFloorType(court.floorType ?? '');
          setIsIndoor(court.isIndoor ?? true);
          setCashOnArrival(court.cashOnArrival ?? false);
          if (court.slotConfig) {
            setAllowedDurations(court.slotConfig.allowedDurations ?? [60]);
            setBufferTime(court.slotConfig.bufferTime ?? 0);
            setMinAdvanceBooking(court.slotConfig.minAdvanceBooking ?? 1);
            setMaxAdvanceBooking(court.slotConfig.maxAdvanceBooking ?? 14);
            setSlotStartTimes(court.slotConfig.slotStartTimes ?? generateSlotTimes());
          }
          if (court.pricing?.pricingRules?.length) {
            setPricingRules(court.pricing.pricingRules);
          } else {
            setPricingRules([
              {
                name: 'Reguler',
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Edit Lapangan</h1>
      </div>
